...
namespace Sexy
{
    ...
    internal class ResourceManager : IDisposable
    {
        ...
        {
            theRes = theRes.Remove(0, 5); // Remove "FONT_" prefix.
            if (this.mFontMap.ContainsKey(theRes))
            {
                return ((FontRes)this.mFontMap[theRes]).mFont;
            }
            throw new Exception(theRes + " not found.");
        }
        ...
        {
            theRes = theRes.Remove(0, 6); // Remove "IMAGE_" prefix.
            if (this.mImageMap.ContainsKey(theRes))
            {
                return ((ImageRes)this.mImageMap[theRes]).mImage;
            }
            return null;// throw new Exception(theRes + " not found.");
        }


        public int GetSoundThrow(string theRes)
        {
            theRes = theRes.Remove(0, 6); // Remove "SOUND_" prefix.
        ...
        private Texture2D LoadTextureFromStream(string filename, bool premultiply, ImageRes.TextureFormat format, SurfaceFormat lowMemorySurfaceFormat)
        {
            ...
            using (Stream stream = TitleContainer.OpenStream("Content/images" + filename + "." + format.ToString().ToLower()))
            {
            ...
            Task<Texture2D> task = new Task<Texture2D>(() => TextureBlendAlpha(texture2D, format, lowMemorySurfaceFormat));
        ...
        public Texture2D TextureBlendAlpha(Texture2D texture2D, ImageRes.TextureFormat format, SurfaceFormat lowMemorySurfaceFormat)
        {
            GraphicsDevice graphicsDevice = GlobalStaticVars.g.GraphicsDevice;
            bool flag = false;
            lock (ResourceManager.DrawLocker)
            {
                if (texture2D.Width * texture2D.Height < 4194304)
                {
                    ...
                    try
                    {
                        ...
                        {
                            Debug.OutputDebug<int, int>(texture2D.Width, texture2D.Height);
        ...
        private bool DoLoadFont(FontRes fontRes)
        {
            ...
            XmlReader xmlReader = XmlReader.Create(TitleContainer.OpenStream("Content/fonts" + fontRes.mPath));
            xmlReader.Read();
