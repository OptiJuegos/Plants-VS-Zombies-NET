...
namespace Sexy
{
	...
	internal class SexyAppBase : SexyAppBaseInterface, ButtonListener, DialogListener, IDisposable
	{
		...
		public bool UpdateApp(GameTime gameTime)
		{
			this.UpdateAudio();
			this.UpdateInput();
		...
		public bool KillDialog(Dialogs theDialogId, bool removeWidget, bool deleteWidget)
		{
			if (this.mDialogMap.ContainsKey((int)theDialogId))
			{
				Dialog dialog = this.mDialogMap[(int)theDialogId];
				this.mDialogList.Remove(dialog);
				this.mDialogMap.Remove((int)theDialogId);
		...
		{
			return this.KillDialog((Dialogs)theDialog.mId);
		}

		
		public void AddDialog(Dialogs theDialogId, Dialog theDialog)
		{
			...
			this.mDialogMap.Add((int)theDialogId, theDialog);
		...
		{
			this.AddDialog((Dialogs)theDialog.mId, theDialog);
		}

		
		public virtual Dialog DoDialog(Dialog theDialog, Dialogs theDialogId)
		{
			this.KillDialog(theDialogId);
		...
		public Dialog GetDialog(Dialogs theDialogId)
		{
			...
			{
				if (keyValuePair.Key == (int)theDialogId)
				{
		...
		public void KeyChar(SexyChar ch)
		{
			this.mWidgetManager.KeyChar(ch);
		}

		public bool MouseMove(int x, int y)
		{
			return mWidgetManager.MouseMove(x, y);
		}

		public void TouchBegan(_Touch touch)
		{
			mIMEHandler.StopTextComposition();	// for IME support, dispose when touching other parts
		...
		public bool PlayMovie(VideoType video, MOVIESCALINGMODE mOVIESCALINGMODE, MOVIECONTROLMODE mOVIECONTROLMODE, SexyColor sexyColor)
		{
			...
			try
			{
				...
				{
					//SexyAppBase.VideoPlayer.Media = new Uri("Content/video/credits.wmv", UriKind.Relative);
		...
		public virtual void UpdateKeys(List<string> keynames) 
		{
		}
		...
		public MonoGame.IMEHelper.IMEHandler mIMEHandler;
	}
}
