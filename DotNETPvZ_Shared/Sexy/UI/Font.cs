...
namespace Sexy
{
	...
	internal class Font
	{
		...
		public int StringWidth(string theString)
		{
			...
			{
				int num = this.ComputeStringWidthForCache(theString);
			}
			return theString.Length * GlobalStaticVars.mGlobalContent.GetLocalizedFontSize();
		}
		...
		{
			return GlobalStaticVars.mGlobalContent.GetLocalizedFontSize();
		}
		...
		public void DrawString(Graphics g, int theX, int theY, string theString, Color theColor)
		{
			...
			SpriteFont font = GlobalStaticVars.mGlobalContent.GetLocalizedFont();
			StringBuilder theSB = new StringBuilder(theString);
			for (int i = 0; i < theString.Length; i++)
			{
				if (!font.Characters.Contains(theString[i]))
				{   // invalid character to render
					theSB[i] = '口';
				}
			}
				g.HardwareClip();
			Graphics.spriteBatch.DrawString(font, theSB, new Vector2((float)theX, (float)theY), theColor, 0f, new Vector2(0f, 0f), GlobalStaticVars.mGlobalContent.GetLocalizedFontScale(), SpriteEffects.None, 0f);
				g.EndHardwareClip();
		}

        //public void DrawString(Graphics g, int theX, int theY, StringBuilder theString, Color theColor)
        //{
        //	DrawString(g, theX, theY, theString.ToString(), theColor);
        //}
        ...
        public void DrawString(Graphics g, int theX, int theY, StringBuilder theString, Color theColor)
        {
            ...
            for (int i = 0; i < this.mFonts.Count; i++)
            {
                ...
                {
                    Vector2 vector = value + this.mOffsets[i] * this.mScaleX * FrameworkConstants.Font_Scale;
                    for (int j = 0; j < theString.Length; j++)
                    {
                        if (char.IsWhiteSpace(theString[j]))
                        {
                            vector.X += (float)this.StringWidth(this.SpaceChar);
                        }
                        else
                        {
                            this.drawStringBuilder[0] = theString[j];
                            SpriteFont localizedFont = GlobalStaticVars.mGlobalContent.GetLocalizedFont();
                            int localizedFontSize = GlobalStaticVars.mGlobalContent.GetLocalizedFontSize();
                            char c = theString[j];
                            if (localizedFont.Characters.Contains(c))
                            {
                                Vector2 position = vector + new Vector2((float)localizedFontSize, (float)localizedFontSize) * this.mScaleX * FrameworkConstants.Font_Scale;
                                bool flag = false;
                                char c2 = ' ';
                                if (this.drawStringBuilder.ToString().length() > 0)
                                {
                                    c2 = this.drawStringBuilder.ToString()[0];
                                    if (c2 == '1' || c2 == '2' || c2 == '3' || c2 == '4' || c2 == '5' || c2 == '6' || c2 == '7' || c2 == '8' || c2 == '9' || c2 == '0' || c2 == ',')
                                    {
                                        position.X += 4f;
                                        position.Y += -3f;
                                        flag = true;
                                ...
                                Graphics.spriteBatch.DrawString(GlobalStaticVars.mGlobalContent.GetLocalizedFont(), this.drawStringBuilder, position, theColor, 0f, new Vector2(0f, 0f), GlobalStaticVars.mGlobalContent.GetLocalizedFontScale(), SpriteEffects.None, 0f);
                                vector.X += (float)(localizedFontSize - this.characterOffsetMagic);
                                if (flag)
                                {
                                    vector.X -= 2f;
                                    if (c2 == ',')
