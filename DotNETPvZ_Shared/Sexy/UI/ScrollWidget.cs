...
namespace Sexy
{
	...
	internal class ScrollWidget : Widget, ProxyWidgetListener
	{
		...
		public override void TouchEnded(_Touch touch)
		{
			...
			{
				CGPoint pointb = this.GetAbsPos() - this.mClientLastDown.GetAbsPos();
				CGPoint pointa = new CGPoint(touch.location.X, touch.location.Y);
				CGPoint pointc = pointa + pointb;
				CGMaths.CGPointTranslate(ref touch.location, pointb.mX, pointb.mY);
				CGMaths.CGPointTranslate(ref touch.previousLocation, pointb.mX, pointb.mY);
