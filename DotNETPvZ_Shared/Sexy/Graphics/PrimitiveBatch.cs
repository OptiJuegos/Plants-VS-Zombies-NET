...
namespace Sexy
{
	...
	internal class PrimitiveBatch : IDisposable
	{
		...
		{
			float offsetX = GlobalStaticVars.gSexyAppBase.mScreenScales.mTransX
			* 480f / (float)this.device.PresentationParameters.BackBufferHeight;
			float offsetY = GlobalStaticVars.gSexyAppBase.mScreenScales.mTransY
				* 800f / (float)this.device.PresentationParameters.BackBufferWidth;
			this.basicEffect.View = Matrix.CreateOrthographicOffCenter(0f - offsetX,
				800 + offsetX/*(float)this.device.PresentationParameters.BackBufferWidth*/,
				480 + offsetY/*(float)this.device.PresentationParameters.BackBufferHeight*/, 
				0f - offsetY, 0f, 1f);
		...
		public void Draw(Image img, TRect destination, TRect source, ref Matrix? transform, Vector2 center, Color colour, bool extraOffset, bool sourceOffsetsUsed, PrimitiveBatchEffects effects)
		{
			...
			if (transform != null)
			{
				Matrix value = transform.Value * GlobalStaticVars.gSexyAppBase.mScreenScales.IdentityMatrix;
			...
			else
			{
				...
				this.Transform = GlobalStaticVars.gSexyAppBase.mScreenScales.IdentityMatrix;
				this.mHasTransform = true;
			}
			this.Texture = img;
