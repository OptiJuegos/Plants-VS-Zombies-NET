...
namespace Sexy
{
	...
	internal class Graphics : GraphicsState
	{
		...
		private protected static bool spritebatchBegan { get; set; }
		...
		public void DrawTriangle(TriVertex p1, Color c1, TriVertex p2, Color c2, TriVertex p3, Color c3, Graphics.DrawMode theDrawMode)
		{
			...
			this.SetupDrawMode(theDrawMode);
			Matrix? transform = GlobalStaticVars.gSexyAppBase.mScreenScales.IdentityMatrix;
			if (Graphics.gTransformStack.Count > 0)
			{
				transform = (Graphics.gTransformStack.Peek().mMatrix == null) ? GlobalStaticVars.gSexyAppBase.mScreenScales.IdentityMatrix : new Matrix?(Graphics.gTransformStack.Peek().mMatrix) * GlobalStaticVars.gSexyAppBase.mScreenScales.IdentityMatrix;
			}
			Graphics.primitiveBatch.Begin(PrimitiveType.TriangleList, this.mTransX, this.mTransY, transform, null, Graphics.NormalSamplerState);
