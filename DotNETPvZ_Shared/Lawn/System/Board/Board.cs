	...
	internal class Board : Widget, ButtonListener
	{
		...
		public bool SaveToFile(Sexy.Buffer b)
		{
			...
			try
			{
				...
				int theLong = this.mGridSquareType.GetLength(1);
				b.WriteLong(length);
				b.WriteLong(theLong);
				for (int num5 = 0; num5 < length; num5++) 
				{
					for (int num6 = 0; num6 < theLong; num6++) 
					{ 
						b.WriteLong((int)this.mGridSquareType[num5, num6]);
					}
				}
				...
				b.WriteLong(base.mLastWMUpdateCount);
				b.WriteLong(this.mLawnMowers.Count);
				for (int num7 = 0; num7 < this.mLawnMowers.Count; num7++)
				{
					this.mLawnMowers[num7].SaveToFile(b); 
				}
				...
				for (int num8 = 0; num8 < this.mPlants.Count; num8++)
				{
					this.mPlants[num8].SaveToFile(b);
				}
				...
				for (int num9 = 0; num9 < this.mProjectiles.Count; num9++) 
				{
					this.mProjectiles[num9].SaveToFile(b);
				}
				b.WriteLong(this.mRiseFromGraveCounter);
				b.WriteLong(this.mRowPickingArray.Length);
				for (int ind = 0; ind < this.mRowPickingArray.Length; ind++)
				{
					this.mRowPickingArray[ind].SaveToFile(b);
				}
				...
				for (int num11 = 0; num11 < this.mZombies.Count; num11++)
				{
					this.mZombies[num11].SaveToFile(b);
				}
				length = this.mZombiesInWave.GetLength(0);
				theLong = this.mZombiesInWave.GetLength(1);
				b.WriteLong(length);
				b.WriteLong(theLong);
				for (int num12 = 0; num12 < length; num12++)
				{
					for (int num13 = 0; num13 < theLong; num13++)
					{
						b.WriteLong((int)this.mZombiesInWave[num12, num13]);
					}
				}
				b.WriteLong(0x309);
			}
		...
		{
			int zombieWAIndex = 0;
			for (int zombieIter = 0; zombieIter < (int)ZombieType.NUM_ZOMBIE_TYPES; zombieIter++)
			{
				Board.aZombieWeightArray[zombieIter].Reset();
				ZombieType zombieType = (ZombieType)zombieIter;
				ZombieDefinition zombieDefinition = Zombie.GetZombieDefinition(zombieType);
				if (this.mZombieAllowed[(int)zombieType])
				{
					...
					Board.aZombieWeightArray[zombieWAIndex].mItem = zombieIter;
					Board.aZombieWeightArray[zombieWAIndex].mWeight = mWeight;
					zombieWAIndex++;
				}
			}
			return (ZombieType)TodCommon.TodPickFromWeightedArray(Board.aZombieWeightArray, zombieWAIndex);
		}
		...
		{
			//Debug.OutputDebug<string>(String.Format("mouse moved: x={0}, y={1}", x, y));
			this.mLastToolX = x;
			this.mLastToolY = y;
			base.MouseMove(x, y);
		...
		public override void Update() 
		{
			...
			{
				this.mCobCannonCursorDelayCounter -= 1;
			}
			if (this.mOutOfMoneyCounter > 0)
			{
				this.mOutOfMoneyCounter -= 1;
			}
			...
			{
				this.mCoinBankFadeCount -= 1;
			}
			if (this.mLevelFadeCount > 0)
			{
				this.mLevelFadeCount -= 1;
			}
			this.UpdateLayers();
			if (this.mTimeStopCounter > 0)
			{
		...
		public void DrawProgressMeter(Graphics g)
		{
			...
			int num2 = num + celWidth / 2;
			int num3 = Constants.Board_ProgressBarText_Pos;
			int num4 = TodCommon.TodAnimateCurve(0, 150, this.mProgressMeterWidth, 0, Constants.UIProgressMeterBarEnd, TodCurves.CURVE_LINEAR);
			TRect theSrcRect = new TRect(celWidth - num4 - 7, celHeight, num4, celHeight);
			TRect theDestRect = new TRect(num + celWidth - num4 - 7, y, num4, celHeight);
			...
			if (this.mApp.mGameMode == GameMode.GAMEMODE_CHALLENGE_BEGHOULED || this.mApp.mGameMode == GameMode.GAMEMODE_CHALLENGE_BEGHOULED_TWIST)
			{
				...
				TodCommon.TodDrawString(g, this.progressMeterString, num2, num3, Resources.FONT_DWARVENTODCRAFT12, theColor, DrawStringJustification.DS_ALIGN_CENTER);
			}
			else if (this.mApp.IsSquirrelLevel())
			{
				...
				TodCommon.TodDrawString(g, theText, num2, num3, Resources.FONT_DWARVENTODCRAFT12, theColor, DrawStringJustification.DS_ALIGN_CENTER);
			}
			else if (this.mApp.mGameMode == GameMode.GAMEMODE_CHALLENGE_SLOT_MACHINE)
			{
				int num5 = TodCommon.ClampInt(this.mSunMoney, 0, 2000);
				if (this.progressMeterStringValue != num5)
				{
					this.progressMeterString = Common.StrFormat_(TodStringFile.TodStringTranslate("[PROGRESS_METER_SUN_SLOT_MACHINE]"), num5, 2000, TodStringFile.TodStringTranslate("[SUN]"));
					this.progressMeterStringValue = num5;
				}
				TodCommon.TodDrawString(g, this.progressMeterString, num2, num3, Resources.FONT_DWARVENTODCRAFT12, theColor, celWidth - 10, DrawStringJustification.DS_ALIGN_CENTER);
			}
			else if (this.mApp.mGameMode == GameMode.GAMEMODE_CHALLENGE_ZOMBIQUARIUM)
			{
				int num6 = TodCommon.ClampInt(this.mSunMoney, 0, 1000);
				string theText2 = Common.StrFormat_(TodStringFile.TodStringTranslate("[PROGRESS_METER_SUN_ZOMBIQUARIUM]"), num6, 1000, TodStringFile.TodStringTranslate("[SUN]"));
				TodCommon.TodDrawString(g, theText2, num2, num3, Resources.FONT_DWARVENTODCRAFT12, theColor, DrawStringJustification.DS_ALIGN_CENTER);
			}
			else if (this.mApp.IsIZombieLevel())
			{
				if (this.progressMeterStringValue != this.mChallenge.mChallengeScore)
			...
			else if (this.ProgressMeterHasFlags())
			{
				...
				for (int i = 1; i <= this.mNumWaves / numWavesPerFlag; i++)
				{
					...
					g.DrawImageCel(AtlasResources.IMAGE_FLAGMETERPARTS, theX, y - num7 - 3, 2, 0);
				}
			}
			...
			{
				int num8 = TodCommon.TodAnimateCurve(0, 150, this.mProgressMeterWidth, 0, Constants.UIProgressMeterHeadEnd, TodCurves.CURVE_LINEAR);
				g.DrawImageCel(AtlasResources.IMAGE_FLAGMETERPARTS, num + celWidth - num8 - 20, y - 3, 0, 0);
			}
		}
		...
		public void ZombiesWon(Zombie aZombie)
		{
			...
			this.mApp.AddDialog(Dialogs.DIALOG_GAME_OVER, theDialog);
			this.mApp.mMusic.StopAllMusic();
		...
		public void DrawLevel(Graphics g)
		{
			...
			int num2 = Constants.UILevelPosition.X - Constants.Board_Offset_AspectRatio_Correction;
			int num3 = Constants.UILevelPosition.Y;
			if (this.mChallenge.mChallengeState == ChallengeState.STATECHALLENGE_ZEN_FADING)
			{
				num3 += TodCommon.TodAnimateCurve(50, 0, this.mChallenge.mChallengeStateCounter, 0, 50, TodCurves.CURVE_EASE_IN_OUT);
			}
			...
			TodCommon.TodDrawString(g, this.mLevelStr, num2 - 24, num3, Resources.FONT_HOUSEOFTERROR16, new SexyColor(224, 187, 98, theAlpha), DrawStringJustification.DS_ALIGN_RIGHT);
		}
		...
		public void DrawUICoinBank(Graphics g)
		{
			...
			int num3 = num + Constants.StoreScreen_Coinbank_TextOffset.X - Resources.FONT_CONTINUUMBOLD14.StringWidth(moneyString);
			num3 += 5 * moneyString.length();
			num2 += 4;
			GlobalStaticVars.mGlobalContent.SetLocalizeFont(Loc_Font.kArial);
			g.DrawString(moneyString, num3, num2 + Constants.StoreScreen_Coinbank_TextOffset.Y);
			g.SetColorizeImages(false);
		}
		...
		public void PickZombieWaves()
		{
			...
			for (int i = 0; i < this.mNumWaves; i++)
			{
		...
		public bool IsZombieWaveDistributionOk()
		{
			...
			int[] array = new int[(int)ZombieType.NUM_ZOMBIE_TYPES];
			for (int i = 0; i < (int)ZombieType.NUM_ZOMBIE_TYPES; i++)
			{
				array[i] = 0;
			}
			...
			for (int l = 0; l < (int)ZombieType.NUM_ZOMBIE_TYPES; l++)
			{
				if ((ZombieType)l != ZombieType.ZOMBIE_YETI && Board.CanZombieSpawnOnLevel((ZombieType)l, this.mLevel) && array[l] == 0)
				{
					return false;
				}
			}
		...
		public bool LoadFromFile(Sexy.Buffer b)
		{
			...
			while (num7 < num5)
			{
				...
				while (true)
				{
					...
					this.mGridSquareType[num7, num8] = (GridSquareType)b.ReadLong();
					num8++;
				}
			}
			...
			base.mLastWMUpdateCount = b.ReadLong();
			int num9 = b.ReadLong();
			this.mLawnMowers.Clear();
			...
			int num15 = b.ReadLong();
			for (int num16 = 0; num16 < num15; num16++)
			{
				this.mRowPickingArray[num16] ??= new TodSmoothArray();
				this.mRowPickingArray[num16].LoadFromFile(b);
			}
			...
			while (num19 < num5)
			{
				...
				while (true)
				{
					...
					this.mZombiesInWave[num19, num20] = (ZombieType)b.ReadLong();
					num20++;
				}
			}
			if (b.ReadLong() != 0x309)
			...
			foreach (Zombie zombie2 in this.mZombies)
			{
				zombie2.LoadingComplete();
			}
			foreach (Plant plant2 in this.mPlants)
			{
				plant2.LoadingComplete();
			}
			foreach (Projectile projectile2 in this.mProjectiles)
			{
				projectile2.LoadingComplete();
			}
			foreach (LawnMower mower2 in this.mLawnMowers)
			{
				mower2.LoadingComplete();
			}
			foreach (Coin coin2 in this.mCoins)
			{
				coin2.LoadingComplete();
			}
			foreach (GridItem item2 in this.mGridItems)
			{
				item2.LoadingComplete();
			}
		...
		private static TodWeightedArray[] aZombieWeightArray = new TodWeightedArray[(int)ZombieType.NUM_ZOMBIE_TYPES];
