...
namespace Lawn
{
	...
	internal class Challenge : StoreListener
	{
		...
		{
			this.mBoard.mZombieAllowed = new bool[100];	//new code
			...
			{
				this.mBoard.mZombieAllowed[(int)ZombieType.ZOMBIE_NEWSPAPER] = true;
			}
			else
			{
				this.mBoard.mZombieAllowed[(int)ZombieType.ZOMBIE_TRAFFIC_CONE] = true;
			}
			int i = Math.Min(this.mSurvivalStage+1, 9);
			while (i > 0)
			{
				ZombieType zombieType = (ZombieType)RandomNumbers.NextNumber((int)ZombieType.NUM_ZOMBIE_TYPES);
				if (this.mBoard.mZombieAllowed[(int)zombieType]){ continue; }
				if (!this.mBoard.StageHasPool() && Board.IsZombieTypePoolOnly(zombieType)) { continue; }
				switch (zombieType) 
				{
					case ZombieType.ZOMBIE_DIGGER:
					case ZombieType.ZOMBIE_DANCER:
						if (this.mBoard.StageHasRoof()) { continue; }
						break;
					case ZombieType.ZOMBIE_ZAMBONI:
						if (this.mBoard.StageHasGraveStones()) { continue; }
						break;
					case ZombieType.ZOMBIE_BUNGEE:
						if (this.mBoard.StageHasRoof() && !this.mApp.IsSurvivalEndless(this.mApp.mGameMode)) { continue; }
						break;
				...
				if (this.mBoard.GetSurvivalFlagsCompleted() < 4  && (zombieType == ZombieType.ZOMBIE_GARGANTUAR || zombieType == ZombieType.ZOMBIE_ZAMBONI)) { continue; }
				if (this.mBoard.GetSurvivalFlagsCompleted() < 10 && (zombieType == ZombieType.ZOMBIE_REDEYE_GARGANTUAR)) { continue; }
				if (!(
							(
								this.mApp.mGameMode != GameMode.GAMEMODE_SURVIVAL_NORMAL_STAGE_1 && this.mApp.mGameMode != GameMode.GAMEMODE_SURVIVAL_NORMAL_STAGE_2 && this.mApp.mGameMode != GameMode.GAMEMODE_SURVIVAL_NORMAL_STAGE_3
							)
							|| zombieType <= ZombieType.ZOMBIE_SNORKEL
						)) { continue; }

				/*if (
						(
							!this.mBoard.mZombieAllowed[(int)zombieType] 
						) && (
							!Board.IsZombieTypePoolOnly(zombieType) || this.mBoard.StageHasPool()
						) && (
							!this.mBoard.StageHasRoof() || (zombieType != ZombieType.ZOMBIE_DIGGER && zombieType != ZombieType.ZOMBIE_DANCER)
						) && (
							!this.mBoard.StageHasGraveStones() || zombieType != ZombieType.ZOMBIE_ZAMBONI
						) && (
							this.mBoard.StageHasRoof() || this.mApp.IsSurvivalEndless(this.mApp.mGameMode) || zombieType != ZombieType.ZOMBIE_BUNGEE
						) && (
							this.mBoard.GetSurvivalFlagsCompleted() >= 4 || (zombieType != ZombieType.ZOMBIE_GARGANTUAR && zombieType != ZombieType.ZOMBIE_ZAMBONI)
						) && (
							this.mBoard.GetSurvivalFlagsCompleted() >= 10 || zombieType != ZombieType.ZOMBIE_REDEYE_GARGANTUAR
						) && (
							(
								this.mApp.mGameMode != GameMode.GAMEMODE_SURVIVAL_NORMAL_STAGE_1 && this.mApp.mGameMode != GameMode.GAMEMODE_SURVIVAL_NORMAL_STAGE_2 && this.mApp.mGameMode != GameMode.GAMEMODE_SURVIVAL_NORMAL_STAGE_3
							) 
						...
						&& zombieType != ZombieType.ZOMBIE_BOBSLED 
						&& zombieType != ZombieType.ZOMBIE_BACKUP_DANCER 
						&& zombieType != ZombieType.ZOMBIE_IMP 
						&& zombieType != ZombieType.ZOMBIE_DUCKY_TUBE 
						&& zombieType != ZombieType.ZOMBIE_PEA_HEAD 
						&& zombieType != ZombieType.ZOMBIE_WALLNUT_HEAD 
						&& zombieType != ZombieType.ZOMBIE_TALLNUT_HEAD 
						&& zombieType != ZombieType.ZOMBIE_JALAPENO_HEAD 
						&& zombieType != ZombieType.ZOMBIE_GATLING_HEAD 
						&& zombieType != ZombieType.ZOMBIE_SQUASH_HEAD 
						&& zombieType != ZombieType.ZOMBIE_YETI
					)
				{*/
		...
		public void WhackAZombieSpawning()
		{
			...
			this.mBoard.mZombieCountDown -= 1;
			int num = 300;
			if (this.mBoard.mZombieCountDown == 100 && this.mBoard.mCurrentWave > 0)
			{
			...
			if (this.mBoard.mZombieCountDown == 5)
			{
				this.mBoard.NextWaveComing();
			}
			if (this.mBoard.mZombieCountDown == 0)
			{
