...
namespace Lawn
{
	...
	internal class Projectile : GameObject
	{
		...
		public void UpdateLobMotion()
		{
			...
			this.mVelZ += this.mAccZ; //3f * 
			if (this.mApp.mGameMode == GameMode.GAMEMODE_CHALLENGE_HIGH_GRAVITY)
			{
				this.mVelZ += this.mAccZ; //3f *
			}
			this.mPosX += this.mVelX;//3f * 
			this.mPosY += this.mVelY;//3f * 
			this.mPosZ += this.mVelZ;//3f * 
		...
		{
			float relativeHeight = this.mShadowY - this.mPosY;
			if ((this.mProjectileType == ProjectileType.PROJECTILE_PEA 
				|| this.mProjectileType == ProjectileType.PROJECTILE_SNOWPEA 
				|| this.mProjectileType == ProjectileType.PROJECTILE_FIREBALL 
				|| this.mProjectileType == ProjectileType.PROJECTILE_SPIKE 
				|| this.mProjectileType == ProjectileType.PROJECTILE_COBBIG) 
				&&
				relativeHeight < 28f)
			{
				this.DoImpact(null);
			...
			if (this.mProjectileType == ProjectileType.PROJECTILE_PUFF && relativeHeight < 0f)
			{
				this.DoImpact(null);
				return;
			}
			if (this.mProjectileType == ProjectileType.PROJECTILE_STAR && relativeHeight < 23f)
			{
				this.DoImpact(null);
				return;
			}
			...
			int theColumn = this.mBoard.PixelToGridXKeepOnBoard((int)this.mPosX + 30, (int)this.mPosY);
			if (this.mBoard.mGridSquareType[theColumn, this.mRow] == GridSquareType.GRIDSQUARE_HIGH_GROUND)
			{
				this.DoImpact(null);
			}
		}
