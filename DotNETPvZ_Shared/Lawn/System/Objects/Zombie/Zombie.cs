...
namespace Lawn
{
    ...
    internal class Zombie : GameObject, IComparable
    {
        ...
        public void EatPlant(Plant thePlant)
        {
            ...
            thePlant.mPlantHealth -= GameConstants.TICKS_BETWEEN_EATS; // 3 * 
        ...
        public void Update()
        {
            ...
            this.mJustGotShotCounter -= 1;
            if (this.mShieldJustGotShotCounter > 0)
            {
                this.mShieldJustGotShotCounter -= 1;
            }
            if (this.mShieldRecoilCounter > 0)
            {
                this.mShieldRecoilCounter -= 1;
            }
        ...
        public void PickBungeeZombieTarget(int theColumn)
        {
                ...
                if (theColumn == -1 || theColumn == j)
                {
                    for (int k = 0; k < Constants.MAX_GRIDSIZEY; k++)
                    {
                        ...
                        if (this.mBoard.GetGraveStoneAt(j, k) == null && this.mBoard.mGridSquareType[j, k] != GridSquareType.GRIDSQUARE_DIRT)
                        {
                            ...
                            {
                                Zombie.aPicks[aPickIndex].mX = j;
                                Zombie.aPicks[aPickIndex].mY = k;
                                Zombie.aPicks[aPickIndex].mWeight = mWeight;
                                aPickIndex++;
                            }
                        }
                    }
            ...
            TodWeightedGridArray todWeightedGridArray = TodCommon.TodPickFromWeightedGridArray(Zombie.aPicks, aPickIndex);
        ...
        public void UpdateZombiePosition()
        {
            ...
            if (this.mZombieHeight == ZombieHeight.HEIGHT_ZOMBIE_NORMAL)
            {
                ...
                {
                    this.mPosY += Math.Min(1f, posYBasedOnRow - this.mPosY); //3f * 
                    if (this.mPosY > posYBasedOnRow)
                    {
                ...
                {
                    this.mPosY -= Math.Min(1f, this.mPosY - posYBasedOnRow);//3f * 
                    if (this.mPosY < posYBasedOnRow)
                    {
                        this.mPosY = posYBasedOnRow;
                    }
        ...
        public void UpdateZombiePogo()
        {
            ...
            Plant plant;
            if (this.mZombiePhase == ZombiePhase.PHASE_POGO_FORWARD_BOUNCE_2 && this.mPhaseCounter == 70)
            {
        ...
        {
            Reanimation reanimation = mApp.ReanimationTryToGet(this.mBodyReanimID);
            if (reanimation == null)
            {
                return;
            }
            g.SetDrawMode(Graphics.DrawMode.DRAWMODE_NORMAL);
            float offsetX = theDrawPos.mImageOffsetX;
            float offsetY = theDrawPos.mImageOffsetY + theDrawPos.mBodyY;
            ZombieType zombieType = this.mZombieType;
            switch (zombieType)
            {
                case ZombieType.ZOMBIE_NORMAL:
                    offsetY -= (float)((int)((float)Constants.Zombie_ClipOffset_Normal * this.mScaleZombie));
                    break;
                case ZombieType.ZOMBIE_FLAG:
                    offsetY -= (float)((int)((float)Constants.Zombie_ClipOffset_Pail * this.mScaleZombie));
                    break;
                case ZombieType.ZOMBIE_TRAFFIC_CONE:
                case ZombieType.ZOMBIE_PAIL:
                    offsetY -= (float)((int)((float)Constants.Zombie_ClipOffset_Pail * this.mScaleZombie));
                ...
                case ZombieType.ZOMBIE_SNORKEL:
                        ...
                        this.mZombiePhase == ZombiePhase.PHASE_SNORKEL_WALKING)
                    {
                        ...
                        {
                            offsetY -= (float)((int)((float)Constants.Zombie_ClipOffset_Snorkel_Dying * this.mScaleZombie));
                        }
                        else
                        {
                            offsetY -= (float)Constants.Zombie_ClipOffset_Snorkel_Dying_Small;
                            this.mUsesClipping = true;
                        }
                    }
                    else if (this.mZombiePhase == ZombiePhase.PHASE_SNORKEL_INTO_POOL && this.mScaleZombie != 1f)
                    {
                        offsetY -= (float)Constants.Zombie_ClipOffset_Snorkel_intoPool_Small;
                    }
                    else
                    {
                        offsetY -= (float)((int)((float)Constants.Zombie_ClipOffset_Snorkel * this.mScaleZombie));
                    }
                    if (this.draggedByTangleKelp)
                    {
                        offsetY -= (float)((int)((float)Constants.Zombie_ClipOffset_Snorkel_Grabbed * this.mScaleZombie));
                        this.mUsesClipping = true;
                        break;
                    }
                    break;
                case ZombieType.ZOMBIE_DOLPHIN_RIDER:
                    offsetY -= (float)((int)((float)Constants.Zombie_ClipOffset_Dolphin_Into_Pool * this.mScaleZombie));
                    break;
                case ZombieType.ZOMBIE_DIGGER:
                    offsetY -= (float)((int)((float)Constants.Zombie_ClipOffset_Digger * this.mScaleZombie));
                    break;
                case ZombieType.ZOMBIE_PEA_HEAD:
                case ZombieType.ZOMBIE_WALLNUT_HEAD:
                case ZombieType.ZOMBIE_JALAPENO_HEAD:
                case ZombieType.ZOMBIE_GATLING_HEAD:
                case ZombieType.ZOMBIE_TALLNUT_HEAD:
                    offsetY -= (float)((int)((float)Constants.Zombie_ClipOffset_PeaHead_InPool * this.mScaleZombie));
                    break;
                default:
                    offsetY -= (float)((int)((float)Constants.Zombie_ClipOffset_Default * this.mScaleZombie));
                    break;
            }
            switch (this.mZombiePhase)
            {
                case ZombiePhase.PHASE_RISING_FROM_GRAVE:
                    if (this.mScaleZombie == 1f)
                    {
                        offsetY += (float)((int)((float)Constants.Zombie_ClipOffset_RisingFromGrave * this.mScaleZombie));
                    }
                    else
                    {
                        offsetY += (float)((int)((float)Constants.Zombie_ClipOffset_RisingFromGrave_Small * this.mScaleZombie));
                    }
                    break;
                case ZombiePhase.PHASE_SNORKEL_INTO_POOL:
                    offsetY -= (float)((int)((float)Constants.Zombie_ClipOffset_Snorkel_Into_Pool * this.mScaleZombie));
                    break;
                case ZombiePhase.PHASE_ZOMBIE_NORMAL:
                    if (this.mInPool)
                    {
                        if (this.mZombieType == ZombieType.ZOMBIE_NORMAL)
                        {
                            if (this.mScaleZombie == 1f)
                            {
                                offsetY += (float)Constants.Zombie_ClipOffset_Normal_In_Pool;
                            }
                            else
                            {
                                offsetY += (float)Constants.Zombie_ClipOffset_Normal_In_Pool_SMALL;
                            }
                        }
                        if (this.mZombieType == ZombieType.ZOMBIE_TRAFFIC_CONE && this.mScaleZombie != 1f)
                        {
                            offsetY += (float)Constants.Zombie_ClipOffset_TrafficCone_In_Pool_SMALL;
                        }
                    }
                    break;
                case ZombiePhase.PHASE_ZOMBIE_DYING:
                    if (this.mInPool)
                    {
                        ...
                        {
                            offsetY += (float)Constants.Zombie_ClipOffset_Flag_In_Pool;
                        }
                    }
                    break;
            }
            if (this.mScaleZombie != 1f
                ...
                && this.mInPool)
            {
                offsetY -= this.mAltitude;
            }
            if (theDrawPos.mClipHeight > GameConstants.CLIP_HEIGHT_LIMIT)
            {
                float num2 = 120f - theDrawPos.mClipHeight + 71f;
                g.SetClipRect((int)((offsetX - 200f) * Constants.S), (int)(offsetY * Constants.S), (int)(520f * Constants.S), (int)(num2 * Constants.S));
            }
        ...
        {
            return this.CanBeChilled()
                && this.mZombiePhase != ZombiePhase.PHASE_POLEVAULTER_IN_VAULT
                && this.mZombiePhase != ZombiePhase.PHASE_DOLPHIN_INTO_POOL
                && this.mZombiePhase != ZombiePhase.PHASE_DOLPHIN_IN_JUMP
                && this.mZombiePhase != ZombiePhase.PHASE_SNORKEL_INTO_POOL
                && !this.IsFlying()
                && this.mZombiePhase != ZombiePhase.PHASE_IMP_GETTING_THROWN
                && this.mZombiePhase != ZombiePhase.PHASE_IMP_LANDING
                && this.mZombiePhase != ZombiePhase.PHASE_BOBSLED_CRASHING
                && this.mZombiePhase != ZombiePhase.PHASE_JACK_IN_THE_BOX_POPPING
                ...
                && !this.IsBouncingPogo()
                && (this.mZombieType != ZombieType.ZOMBIE_BUNGEE || this.mZombiePhase == ZombiePhase.PHASE_BUNGEE_AT_BOTTOM);
        }
        ...
        public void UpdateZombieGatlingHead()
        {
            ...
            if (this.mPhaseCounter == 18 || this.mPhaseCounter == 35 || this.mPhaseCounter == 51 || this.mPhaseCounter == 68)
            {
                this.mApp.PlayFoley(FoleyType.FOLEY_THROW);
                Reanimation reanimation2 = this.mApp.ReanimationGet(this.mBodyReanimID);
                if (reanimation2 != null)
