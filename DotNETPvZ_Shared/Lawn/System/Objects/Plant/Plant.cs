...
namespace Lawn
{
    ...
    internal class Plant : GameObject
    {
        ...
        protected override void Reset()
        {
            ...
            this.mLaunchRate = 0;
            this.mPlantRect = default;
            this.mPlantAttackRect = default;
            this.mTargetX = 0;
            this.mTargetY = 0;
            this.mStartRow = 0;
        ...
        public void Animate()
        {
            ...
            {
                this.mRecentlyEatenCountdown -= 1;
            }
            if (this.mEatenFlashCountdown > 0)
            {
                this.mEatenFlashCountdown -= 1;
            }
            if (this.mBeghouledFlashCountdown > 0)
            {
                this.mBeghouledFlashCountdown -= 1;
            }
            if (this.mSquished)
            {
                this.mFrame = 0;
        ...
        public void Draw(Graphics g)
        {
            ...
            if (this.IsOnBoard())
            {
                ...
                if (plant != null)
                {
                    Plant theTopPlant = this.mBoard.GetTopPlantAt(this.mPlantCol, this.mRow, PlantPriority.TOPPLANT_ONLY_NORMAL_POSITION);
                    if (theTopPlant != null && theTopPlant.mRenderOrder > plant.mRenderOrder)
                    {
                        theTopPlant = null;
                    }
                    if (theTopPlant != null && theTopPlant.mOnBungeeState == PlantOnBungeeState.PLANT_GETTING_GRABBED_BY_BUNGEE)
                    {
                        theTopPlant = null;
                    }
                    if (theTopPlant == this)
                    ...
                    if (theTopPlant == null && this.mSeedType == SeedType.SEED_PUMPKINSHELL)
                    {
                        flag = true;
                    }
                }
        ...
        public Zombie FindTargetZombie(int theRow, PlantWeapon thePlantWeapon)
        {
            ...
            {
                Zombie it = this.mBoard.mZombies[i];
                if (it.mDead)
                {
                    continue;
                }
                int diffInRow = it.mRow - theRow;
                if (it.mZombieType == ZombieType.ZOMBIE_BOSS)
                {
                    diffInRow = 0;
                }
                if ((it.mHasHead && !it.IsTangleKelpTarget()) || (this.mSeedType != SeedType.SEED_POTATOMINE && this.mSeedType != SeedType.SEED_CHOMPER && this.mSeedType != SeedType.SEED_TANGLEKELP))
                {
                    bool portalModeAndLinearBullet = false;
                    ...
                    {
                        portalModeAndLinearBullet = true;
                    }
                    if (this.mSeedType != SeedType.SEED_CATTAIL)
                    {
                        if (this.mSeedType == SeedType.SEED_GLOOMSHROOM)
                        {
                            if (diffInRow < -1 || diffInRow > 1)
                            {
                                continue;
                            }
                        }
                        else if (portalModeAndLinearBullet)
                        {
                            if (!this.mBoard.mChallenge.CanTargetZombieWithPortals(this, it))
                            {
                                continue;
                            }
                        }
                    ...
                    if (it.EffectedByDamage((uint)damageRangeFlags))
                    {
                        int num3 = 0;
                        if (this.mSeedType == SeedType.SEED_CATTAIL)
                        {
                        ...
                        {
                            if (it.mZombiePhase == ZombiePhase.PHASE_DIGGER_WALKING)
                            {
                                plantAttackRect.mX += 20;
                                plantAttackRect.mWidth -= 20;
                            }
                            if (it.mZombiePhase == ZombiePhase.PHASE_POGO_BOUNCING || (it.mZombieType == ZombieType.ZOMBIE_BUNGEE && it.mTargetCol == this.mPlantCol))
                            {
                                continue;
                            }
                            if (it.mIsEating || this.mState == PlantState.STATE_CHOMPER_BITING)
                            {
                                num3 = 60;
                            }
                        }
                        if (this.mSeedType == SeedType.SEED_POTATOMINE)
                        {
                            if ((it.mZombieType == ZombieType.ZOMBIE_POGO && it.mHasObject) || it.mZombiePhase == ZombiePhase.PHASE_POLEVAULTER_IN_VAULT || it.mZombiePhase == ZombiePhase.PHASE_POLEVAULTER_PRE_VAULT)
                            {
                                continue;
                            }
                            if (it.mZombieType == ZombieType.ZOMBIE_POLEVAULTER)
                            {
                                plantAttackRect.mX += 40;
                                plantAttackRect.mWidth -= 40;
                            }
                            if (it.mZombieType == ZombieType.ZOMBIE_BUNGEE && it.mTargetCol != this.mPlantCol)
                            {
                                continue;
                            }
                            if (it.mIsEating)
                        ...
                        if ((this.mSeedType != SeedType.SEED_EXPLODE_O_NUT || it.mZombiePhase != ZombiePhase.PHASE_POLEVAULTER_IN_VAULT) && (this.mSeedType != SeedType.SEED_TANGLEKELP || it.mInPool))
                        {
                            TRect zombieRect = it.GetZombieRect();
                            if (!portalModeAndLinearBullet)
                            {
        ...
        public void UpdateAbilities()
        {
            ...
            {
                this.mWakeUpCounter -= 1;
                if (this.mWakeUpCounter == 60)
                {
        ...
        public void DoRowAreaDamage(int theDamage, uint theDamageFlags)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (!zombie.mDead)
                {
                    int diffInRow = zombie.mRow - this.mRow;
        ...
        public void GetPeaHeadOffset(ref int theOffsetX, ref int theOffsetY)
        {
            ...
            reanimation.GetCurrentTransform(theTrackIndex, out ReanimatorTransform reanimatorTransform, false);
        ...
        public void UpdateShooting()
        {
            ...
            this.mShootingCounter -= 1;
            if (this.mSeedType == SeedType.SEED_FUMESHROOM && this.mShootingCounter == 15)
            {
            ...
            if (this.mSeedType == SeedType.SEED_GLOOMSHROOM)
            {
                if ((this.mShootingCounter == 136) || (this.mShootingCounter == 108) || (this.mShootingCounter == 80) || (this.mShootingCounter == 52))
                {
                ...
                if ((this.mShootingCounter == 126) || (this.mShootingCounter == 98) || (this.mShootingCounter == 70) || (this.mShootingCounter == 42))
                {
            ...
            {
                if ((this.mShootingCounter == 18) || (this.mShootingCounter == 35) || (this.mShootingCounter == 51) || (this.mShootingCounter == 68))
                {
                    this.Fire(null, this.mRow, PlantWeapon.WEAPON_PRIMARY);
                }
            }
            ...
            else if (this.mShootingCounter == 1)
            {
                if (this.mSeedType == SeedType.SEED_THREEPEATER)
                {
                    int theRow = this.mRow - 1;
        ...
        public void MagnetShroomAttactItem(Zombie theZombie)
        {
            ...
            else if (theZombie.mZombieType == ZombieType.ZOMBIE_POGO)
            {
                ...
                ZombieDrawPosition zombieDrawPosition = default;
        ...
        public TRect mPlantRect = default;


        public TRect mPlantAttackRect = default;
        ...
        private static readonly Stack<Plant> unusedObjects = new Stack<Plant>();
