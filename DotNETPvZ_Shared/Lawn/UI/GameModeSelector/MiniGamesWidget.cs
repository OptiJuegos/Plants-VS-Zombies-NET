...
namespace Lawn
{
	...
	internal class MiniGamesWidget : Widget
	{
		...
		public MiniGamesWidget(LawnApp theApp, MiniGamesWidgetListener theListener)
		{
			...
			this.mWidth = 0;
			this.mHeight = AtlasResources.IMAGE_MINI_GAME_FRAME.mHeight + 50;
		...
		public override void Draw(Graphics g)
		{
			...
			for (int j = 0; j < modeLevelCount; j++)
			{
				...
				g.WriteWordWrapped(new TRect(num - 9, AtlasResources.IMAGE_MINI_GAME_FRAME.mHeight, AtlasResources.IMAGE_MINI_GAME_FRAME.mWidth - 30, 150), levelName, 5, 0);
				num += AtlasResources.IMAGE_MINI_GAME_FRAME.mWidth + 10;
			}
			if (drawPadlock)
			{
		...
		public void DisplayLockedMessage()
		{
			switch (this.mMode)
			{
				case MiniGameMode.MINI_GAME_MODE_GAMES:
					...
					{
						this.mApp.LawnMessageBox(Dialogs.DIALOG_MESSAGE, "[MODE_LOCKED]", "[FINISH_ADVENTURE_TOOLTIP]", "[DIALOG_BUTTON_OK]", "", 3, null);
						return;
					}
					this.mApp.LawnMessageBox(Dialogs.DIALOG_MESSAGE, "[MODE_LOCKED]", "[ONE_MORE_CHALLENGE_TOOLTIP]", "[DIALOG_BUTTON_OK]", "", 3, null);
					return;
				case MiniGameMode.MINI_GAME_MODE_I_ZOMBIE:
					...
					this.mApp.LawnMessageBox(Dialogs.DIALOG_MESSAGE, "[MODE_LOCKED]", "[ONE_MORE_IZOMBIE_TOOLTIP]", "[DIALOG_BUTTON_OK]", "", 3, null);
					return;
				case MiniGameMode.MINI_GAME_MODE_VASEBREAKER:
					...
					this.mApp.LawnMessageBox(Dialogs.DIALOG_MESSAGE, "[MODE_LOCKED]", "[ONE_MORE_SCARY_POTTER_TOOLTIP]", "[DIALOG_BUTTON_OK]", "", 3, null);
					return;
				default:
					return;
			}
