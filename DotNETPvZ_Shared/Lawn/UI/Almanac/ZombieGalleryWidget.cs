...
namespace Lawn
{
	...
	internal class ZombieGalleryWidget : Widget
	{
		...
		{
			if (theIndex >= (int)ZombieType.NUM_ZOMBIE_TYPES)
			{
		...
		public ZombieType ZombieHitTest(int x, int y)
		{
			...
			{
				if (i < (int)ZombieType.NUM_ZOMBIE_TYPES)
				{
			...
			for (int i = 0; i < GameConstants.NUM_ALMANAC_ZOMBIES; i++)
			{
				...
				if (zombieType != ZombieType.ZOMBIE_INVALID)
				{
					...
					else
					{
						...
						@new.ClipRect(
							new TRect(num + Constants.ZombieGalleryWidget_Window_Clip.X, num2 + Constants.ZombieGalleryWidget_Window_Clip.Y, Constants.ZombieGalleryWidget_Window_Clip.Width, Constants.ZombieGalleryWidget_Window_Clip.Height)
							);
