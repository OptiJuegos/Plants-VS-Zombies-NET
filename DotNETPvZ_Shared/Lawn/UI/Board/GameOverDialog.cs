...
namespace Lawn
{
	...
	internal class GameOverDialog : LawnDialog
	{
		...
		public GameOverDialog(string theMessage, bool theShowChallengeName) : base((LawnApp)GlobalStaticVars.gSexyAppBase, null, 17, true, "[GAME_OVER]", theMessage, "", 3)
		{
			...
			this.mLawnYesButton.Resize(this.mLawnYesButton.mX - 10, this.mLawnYesButton.mY, this.mLawnYesButton.mWidth + 30, this.mLawnYesButton.mHeight);
			this.mClip = false;
		...
		public override void ButtonDepress(int theId)
		{
			if (theId != 1)
			{
				if (theId == 1000)
				{
					this.mApp.KillDialog(Dialogs.DIALOG_GAME_OVER);
					this.mApp.EndLevel();
