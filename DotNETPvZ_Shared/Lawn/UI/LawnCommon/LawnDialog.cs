...
namespace Lawn
{
	...
	internal class LawnDialog : Dialog
	{
		...
		{
			int totalWidth = this.mContentInsets.mLeft + this.mContentInsets.mRight + this.mBackgroundInsets.mLeft + this.mBackgroundInsets.mRight + theExtraX;
			if (this.mDialogHeader.Length > 0)
			{
				totalWidth += this.mHeaderFont.StringWidth(this.mDialogHeader);
				if (totalWidth < theMinWidth)
				{
					totalWidth = theMinWidth;
				}
			}
			...
			{
				totalWidth += AtlasResources.IMAGE_DIALOG_TOPMIDDLE.mWidth;
				Graphics @new = Graphics.GetNew();
				@new.SetFont(this.mLinesFont);
				num3 += this.GetWordWrappedHeight(@new, totalWidth - this.mContentInsets.mLeft - this.mContentInsets.mRight - this.mBackgroundInsets.mLeft - this.mBackgroundInsets.mRight - (int)Constants.InvertAndScale(4f), this.mDialogLines, this.mLineSpacingOffset);
				@new.PrepareForReuse();
			}
			...
			this.Resize(this.mX, this.mY, totalWidth, num3);
		}
