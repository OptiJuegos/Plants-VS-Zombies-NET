...
namespace Lawn
{
	...
	internal class NewUserDialog : LawnDialog, EditListener
	{
		...
		public override void Resize(int theX, int theY, int theWidth, int theHeight)
		{
			...
			this.mNameEditWidget.Resize(this.mContentInsets.mLeft + (int)Constants.InvertAndScale(12f),
				(int)Constants.InvertAndScale((float)(this.mIsRename ? 143 : 143)), // 170 if no button will be shown
				this.mWidth - this.mContentInsets.mLeft - this.mContentInsets.mRight - (int)Constants.InvertAndScale(24f),
				(int)Constants.InvertAndScale(28f));
		}
		...
		{
			base.Draw(g);
			LawnCommon.DrawEditBox(g, this.mNameEditWidget);
		}
