	...
	internal class CutScene
	{
		...
		public void AnimateBoard()
		{
			...
			if (this.mCutsceneTime > num && this.mCutsceneTime <= num2)
			{
				...
				int leftDisplacement = this.CalcPosition(num, num2, thePositionStart, thePositionEnd);
				this.mBoard.Move(-(int)((float)leftDisplacement * Constants.S), 0);
			}
		...
		public void PlaceStreetZombies()
		{
			...
			if (this.mApp.mGameMode == GameMode.GAMEMODE_CHALLENGE_LAST_STAND)
			{
				...
				{
					if ((ZombieType)l != ZombieType.ZOMBIE_YETI && this.mBoard.mZombieAllowed[l])
					{
			...
			for (int num4 = 0; num4 < (int)ZombieType.NUM_ZOMBIE_TYPES; num4++)
			{
				if (array[num4] != 0 && (this.Is2x2Zombie((ZombieType)num4) || (ZombieType)num4 == ZombieType.ZOMBIE_ZAMBONI))
				{
					this.FindAndPlaceZombie((ZombieType)num4, array2);
				}
			}
			for (int num5 = 0; num5 < (int)ZombieType.NUM_ZOMBIE_TYPES; num5++)
			{
				if (array[num5] != 0 && !this.Is2x2Zombie((ZombieType)num5) && (ZombieType)num5 != ZombieType.ZOMBIE_ZAMBONI)
				{
		...
		public void UpdateZombiesWon()
		{
			...
			if (this.mCutsceneTime == CutScene.LostTimeEnd)
			{
				...
				this.mApp.AddDialog(Dialogs.DIALOG_GAME_OVER, theDialog2);
			}
		}
		...
		public void AdvanceCrazyDaveDialog(bool theJustSkipping)
		{
			...
			if (flag && !theJustSkipping)
			{
				...
				string theDialogHeader = TodCommon.TodReplaceString("[UPGRADE_SPEND]", "{COIN}", LawnApp.GetMoneyString(itemCost));
				LawnDialog lawnDialog = this.mApp.DoDialog(Dialogs.DIALOG_PURCHASE_PACKET_SLOT, true, theDialogHeader, theDialogLines, string.Empty, 1);
		...
		public void PreloadResources()
		{
			...
			for (int k = 0; k < (int)SeedType.NUM_SEED_TYPES; k++)
			{
