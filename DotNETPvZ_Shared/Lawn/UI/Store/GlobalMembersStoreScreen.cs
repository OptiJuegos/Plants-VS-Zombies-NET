	...
	internal static class GlobalMembersStoreScreen
	{
		...
		internal static void DrawStoreItemIcon(Graphics g, int aPosX, int aPosY, StoreItem theItemType, int aQuantity)
		{
			if (theItemType == StoreItem.STORE_ITEM_PACKET_UPGRADE)
			{
				...
				theRect.mX -= 18;
				TodStringFile.TodDrawStringWrapped(g, text, theRect, Resources.FONT_BRIANNETOD12, SexyColor.Black, DrawStringJustification.DS_ALIGN_CENTER_VERTICAL_MIDDLE);
				return;
			}
		...
		internal static void DrawStoreItem(Graphics g, int aPosX, int aPosY, StoreItem theItemType, bool isComingSoon, bool isSoldOut, int aQuantity, int aCost)
		{
			...
			GlobalStaticVars.mGlobalContent.SetLocalizeFont(Loc_Font.kArial);
			TodCommon.TodDrawString(g, moneyString, aPosX + 30 + Constants.StoreScreen_PriceTag_Text_Offset_X, aPosY + Constants.StoreScreen_PriceTag_Text_Offset_Y, Resources.FONT_BRIANNETOD12, new SexyColor(0, 0, 0), DrawStringJustification.DS_ALIGN_CENTER);
			if (isComingSoon)
			{
				...
				TodStringFile.TodDrawStringWrapped(g, "[COMING_SOON]", theRect, Resources.FONT_BRIANNETOD12, new SexyColor(255, 0, 0), DrawStringJustification.DS_ALIGN_CENTER_VERTICAL_MIDDLE);
				return;
			}
			if (isSoldOut)
			{
				...
				GlobalStaticVars.mGlobalContent.SetLocalizedFontSize(localizedFontSize + 3);
				TodStringFile.TodDrawStringWrapped(g, "[SOLD_OUT]", theRect2, Resources.FONT_BRIANNETOD12, new SexyColor(255, 0, 0), DrawStringJustification.DS_ALIGN_CENTER_VERTICAL_MIDDLE);
