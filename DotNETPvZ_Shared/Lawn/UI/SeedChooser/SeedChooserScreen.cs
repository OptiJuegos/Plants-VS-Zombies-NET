...
namespace Lawn
{
	...
	internal class SeedChooserScreen : Widget, StoreListener, AlmanacListener, LawnMessageBoxListener, SeedPacketsWidgetListener
	{
		...
		public SeedChooserScreen()
		{
			...
			this.mStartButton.mTextOffsetX = 0;
			this.mStartButton.mTextOffsetX = -5;
			this.mStartButton.mTextOffsetY = 5;
			...
			this.mAlmanacButton.mTextOffsetX = -7;
			this.mAlmanacButton.mTextOffsetY = 0;
			...
			this.mStoreButton.mTextOffsetX = -7;
			this.mStoreButton.mTextOffsetY = 0;
		...
		public override void MouseDown(int x, int y, int theClickCount)
		{
			...
			this.mApp.AddDialog(Dialogs.DIALOG_IMITATER, imitaterDialog);
		...
		public void ClickedSeedInChooser(ref ChosenSeed theChosenSeed)
		{
			...
			if (this.mApp.mPlayerInfo.mNeedsGrayedPlantWarning)
			{
				...
				if (num != 0U)
				{
					...
					this.mApp.DoDialog(Dialogs.DIALOG_INFO, true, "[DIALOG_WARNING]", "[NOT_RECOMMENDED_FOR_LEVEL]", "[DIALOG_BUTTON_OK]", 3);
					return;
				}
			}
		...
		public bool DisplayRepickWarningDialog(int theWarningId, string theMessage)
		{
			...
			this.mApp.LawnMessageBox(Dialogs.DIALOG_CHOOSER_WARNING, "[DIALOG_WARNING]", theMessage, "[DIALOG_BUTTON_YES]", "[REPICK_BUTTON]", 1, this);
			return false;
		}
		...
		public void SeedSelected(SeedType theSeedType)
		{
			...
			{
				this.mApp.DoDialog(Dialogs.DIALOG_INFO, true, string.Empty, "[NOT_ALLOWED_ON_THIS_LEVEL]", "[DIALOG_BUTTON_OK]", 3);
				return;
			}
		...
		public ChosenSeed[] mChosenSeeds = new ChosenSeed[(int)SeedType.NUM_SEED_TYPES];
		...
		private static TodWeightedArray[] aSeedArray = new TodWeightedArray[(int)SeedType.NUM_SEED_TYPES];
	}
}
