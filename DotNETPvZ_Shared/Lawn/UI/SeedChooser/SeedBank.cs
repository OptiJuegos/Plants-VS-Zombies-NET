using System;
using Sexy;
using Sexy.TodLib;

namespace Lawn
{
	
	internal class SeedBank : GameObject
	{
		
		public SeedBank()
		{
			this.mWidth = (int)Constants.InvertAndScale(62f);
			this.mHeight = this.mApp.mHeight;
			this.mNumPackets = 0;
			this.mCutSceneDarken = 255;
			this.mConveyorBeltCounter = 0;
			this.mPosScaled = false;
			for (int i = 0; i < this.mSeedPackets.Length; i++)
			{
				this.mSeedPackets[i] = new SeedPacket();
			}
		}

		
		public void Draw(Graphics g)
		{
			if (this.mBoard.mCutScene != null && this.mBoard.mCutScene.IsBeforePreloading())
			{
				return;
			}
			if (this.mApp.mGameScene != GameScenes.SCENE_PLAYING)
			{
				g.mTransX -= this.mBoard.mX;
				g.mTransY -= this.mBoard.mY;
			}
			else
			{
				g.mTransX -= Constants.Board_Offset_AspectRatio_Correction;
			}
			if (!this.mApp.IsSlotMachineLevel() && this.mBoard.HasConveyorBeltSeedBank())
			{
				for (int i = -(this.mConveyorBeltCounter / 4) % Resources.IMAGE_CONVEYORBELT_BELT.mHeight; i < this.mHeight; i += Resources.IMAGE_CONVEYORBELT_BELT.mHeight)
				{
					g.DrawImage(Resources.IMAGE_CONVEYORBELT_BELT, 0, i);
				}
				g.DrawImage(Resources.IMAGE_CONVEYORBELT_BACKDROP, 0, 0);
				g.SetClipRect(ref Constants.ConveyorBeltClipRect);
			}
			for (int j = 0; j < this.mNumPackets; j++)
			{
				SeedPacket seedPacket = this.mSeedPackets[j];
				if (seedPacket.mPacketType != SeedType.SEED_NONE && seedPacket.BeginDraw(g))
				{
					seedPacket.DrawBackground(g);
					seedPacket.EndDraw(g);
				}
			}
			for (int k = 0; k < this.mNumPackets; k++)
			{
				SeedPacket seedPacket2 = this.mSeedPackets[k];
				if (seedPacket2.mPacketType != SeedType.SEED_NONE && seedPacket2.BeginDraw(g))
				{
					seedPacket2.Draw(g);
					seedPacket2.EndDraw(g);
				}
			}
			for (int l = 0; l < this.mNumPackets; l++)
			{
				SeedPacket seedPacket3 = this.mSeedPackets[l];
				if (seedPacket3.mPacketType != SeedType.SEED_NONE && seedPacket3.BeginDraw(g))
				{
					seedPacket3.DrawOverlay(g);
					seedPacket3.EndDraw(g);
				}
			}
			g.ClearClipRect();
			this.mApp.IsSlotMachineLevel();
			if (this.mApp.mGameScene != GameScenes.SCENE_PLAYING)
			{
				g.mTransX += this.mBoard.mX;
				g.mTransY += this.mBoard.mY;
			}
		}

		
		public override bool SaveToFile(Sexy.Buffer b)
		{
			try
			{
				base.SaveToFile(b);
				b.WriteLong(this.mConveyorBeltCounter);
				b.WriteLong(this.mCutSceneDarken);
				b.WriteLong(this.mNumPackets);
				for (int i = 0; i < this.mSeedPackets.Length; i++)
				{
					b.WriteBoolean(this.mSeedPackets[i] != null);
					if (this.mSeedPackets[i] != null)
					{
						this.mSeedPackets[i].SaveToFile(b);
					}
				}
			}
			catch (Exception ex)
			{
				string message = ex.Message;
				return false;
			}
			return true;
		}

		
		public override bool LoadFromFile(Sexy.Buffer b)
		{
			try
			{
				base.LoadFromFile(b);
				this.mConveyorBeltCounter = b.ReadLong();
				this.mCutSceneDarken = b.ReadLong();
				this.mNumPackets = b.ReadLong();
				for (int i = 0; i < this.mSeedPackets.Length; i++)
				{
					bool flag = b.ReadBoolean();
					if (flag)
					{
						this.mSeedPackets[i] = new SeedPacket();
						this.mSeedPackets[i].LoadFromFile(b);
					}
					else
					{
						this.mSeedPackets[i] = null;
					}
				}
			}
			catch (Exception ex)
			{
				string message = ex.Message;
				return false;
			}
			return true;
		}

		
		public void DrawSun(Graphics g)
		{
			if (this.mBoard.mCutScene != null && (this.mBoard.mCutScene.IsBeforePreloading() || this.mApp.mGameMode == GameMode.GAMEMODE_INTRO))
			{
				return;
			}
			if (!this.mBoard.HasConveyorBeltSeedBank())
			{
				int i = Math.Max(this.mBoard.mSunMoney, 0);
				string theText = LawnApp.ToString(i);
				SexyColor theColor = new SexyColor(0, 0, 0);
				if (this.mBoard.mOutOfMoneyCounter > 0 && this.mBoard.mOutOfMoneyCounter % 20 < 10)
				{
					theColor = new SexyColor(255, 0, 0);
				}
				int num = Constants.UISunBankPositionX - Constants.Board_Offset_AspectRatio_Correction;
				int mX = this.mX;
				g.DrawImage(AtlasResources.IMAGE_DAN_SUNBANK, num, mX);
				GlobalStaticVars.mGlobalContent.SetLocalizeFont(Loc_Font.kArial);
				TodCommon.TodDrawString(g, theText, num + Constants.UISunBankTextOffset.X, mX + 5 + Constants.UISunBankTextOffset.Y, Resources.FONT_CONTINUUMBOLD14OUTLINE, theColor, DrawStringJustification.DS_ALIGN_CENTER);
			}
		}

		
		public bool MouseHitTest(int x, int y, out HitResult theHitResult)
		{
			theHitResult = default(HitResult);
			if ((float)x > (float)this.mWidth * Constants.S - 50f)
			{
				return false;
			}
			SeedPacket seedPacket = null;
			int num = Constants.SMALL_SEEDPACKET_HEIGHT * 2;
			for (int i = 0; i < this.mNumPackets; i++)
			{
				SeedPacket seedPacket2 = this.mSeedPackets[i];
				if (seedPacket2.mPacketType != SeedType.SEED_NONE)
				{
					int num2 = seedPacket2.CenterY();
					if (num2 <= this.mHeight)
					{
						int num3 = Math.Abs(num2 - y);
						if (num3 < num)
						{
							num = num3;
							seedPacket = seedPacket2;
						}
					}
				}
			}
			if (seedPacket != null)
			{
				theHitResult.mObject = seedPacket;
				theHitResult.mObjectType = GameObjectType.OBJECT_TYPE_SEEDPACKET;
				return true;
			}
			theHitResult.mObject = null;
			theHitResult.mObjectType = GameObjectType.OBJECT_TYPE_NONE;
			return false;
		}

		
		public void Move(int x, int y)
		{
			this.mX = x - ((this.mApp.mGameScene != GameScenes.SCENE_PLAYING) ? 0 : Constants.Board_Offset_AspectRatio_Correction);
			this.mY = y;
		}

		
		public bool ContainsPoint(int theX, int theY)
		{
			return theX >= this.mX && theX < this.mX + this.mWidth && theY >= this.mY && theY < this.mY + this.mHeight;
		}

		
		public int GetNumSeedsOnConveyorBelt()
		{
			for (int i = 0; i < this.mNumPackets; i++)
			{
				SeedPacket seedPacket = this.mSeedPackets[i];
				if (seedPacket.mPacketType == SeedType.SEED_NONE)
				{
					return i;
				}
			}
			return this.mNumPackets;
		}

		
		public void AddSeed(SeedType theSeedType)
		{
			Debug.ASSERT(this.mBoard.HasConveyorBeltSeedBank());
			Debug.ASSERT(theSeedType != SeedType.SEED_NONE);
			int numSeedsOnConveyorBelt = this.GetNumSeedsOnConveyorBelt();
			if (numSeedsOnConveyorBelt == this.mNumPackets)
			{
				return;
			}
			SeedPacket seedPacket = this.mSeedPackets[numSeedsOnConveyorBelt];
			seedPacket.mPacketType = theSeedType;
			seedPacket.mRefreshCounter = 0;
			seedPacket.mRefreshTime = 0;
			seedPacket.mRefreshing = false;
			seedPacket.mActive = true;
			seedPacket.mOffsetY = (int)Constants.InvertAndScale(400f) - 35 * numSeedsOnConveyorBelt;
			if (numSeedsOnConveyorBelt > 0)
			{
				SeedPacket seedPacket2 = this.mSeedPackets[numSeedsOnConveyorBelt - 1];
				if (seedPacket.mOffsetY < seedPacket2.mOffsetY)
				{
					seedPacket.mOffsetY = seedPacket2.mOffsetY + 40;
				}
			}
		}

		
		public void RemoveSeed(int theIndex)
		{
			Debug.ASSERT(this.mBoard.HasConveyorBeltSeedBank());
			Debug.ASSERT(theIndex >= 0 && theIndex < this.GetNumSeedsOnConveyorBelt());
			for (int i = theIndex; i < this.mNumPackets; i++)
			{
				SeedPacket seedPacket = this.mSeedPackets[i];
				if (seedPacket.mPacketType == SeedType.SEED_NONE)
				{
					return;
				}
				if (i == this.mNumPackets - 1)
				{
					seedPacket.mPacketType = SeedType.SEED_NONE;
					seedPacket.mOffsetY = 0;
				}
				else
				{
					SeedPacket seedPacket2 = this.mSeedPackets[i + 1];
					seedPacket.mPacketType = seedPacket2.mPacketType;
					seedPacket.mOffsetY = seedPacket2.mOffsetY + Constants.SMALL_SEEDPACKET_HEIGHT;
				}
				seedPacket.mRefreshCounter = 0;
				seedPacket.mRefreshTime = 0;
				seedPacket.mRefreshing = false;
				seedPacket.mActive = true;
			}
		}

		
		public int CountOfTypeOnConveyorBelt(SeedType aSeedType)
		{
			int num = 0;
			for (int i = 0; i < this.mNumPackets; i++)
			{
				SeedPacket seedPacket = this.mSeedPackets[i];
				if (seedPacket.mPacketType == aSeedType)
				{
					num++;
				}
			}
			return num;
		}

		
		public void UpdateConveyorBelt()
		{
			this.mConveyorBeltCounter++;
			if (this.mConveyorBeltCounter % 4 == 0)
			{
				for (int i = 0; i < this.mNumPackets; i++)
				{
					SeedPacket seedPacket = this.mSeedPackets[i];
					if (seedPacket.mOffsetY > 0)
					{
						seedPacket.mOffsetY = Math.Max(seedPacket.mOffsetY - 1, 0);
					}
				}
			}
		}

		
		public void UpdateHeight()
		{
			this.mNumPackets = this.mBoard.GetNumSeedsInBank();
			this.mHeight = this.mApp.mHeight;
			for (int i = 0; i < this.mNumPackets; i++)
			{
				SeedPacket seedPacket = this.mSeedPackets[i];
				seedPacket.mY = this.mBoard.GetSeedPacketPositionY(i);
			}
		}

		
		public void RefreshAllPackets()
		{
			for (int i = 0; i < this.mNumPackets; i++)
			{
				SeedPacket seedPacket = this.mSeedPackets[i];
				if (seedPacket.mPacketType == SeedType.SEED_NONE)
				{
					return;
				}
				if (seedPacket.mRefreshing)
				{
					seedPacket.mRefreshCounter = 0;
					seedPacket.mRefreshing = false;
					seedPacket.Activate();
					seedPacket.FlashIfReady();
				}
			}
		}

		
		public int mNumPackets;

		
		public SeedPacket[] mSeedPackets = new SeedPacket[9];

		
		public int mCutSceneDarken;

		
		public int mConveyorBeltCounter;
	}
}
