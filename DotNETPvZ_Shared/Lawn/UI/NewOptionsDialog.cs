...
namespace Lawn
{
	...
	internal class NewOptionsDialog : LawnDialog, SliderListener, CheckboxListener
	{
		...
		public NewOptionsDialog(LawnApp theApp, bool theFromGameSelector) : base(theApp, null, 2, true, "[OPTIONS_DIALOG_TITLE]", "", "", 0)
		{
			...
			this.mCheatCheckBox = LawnCommon.MakeNewCheckbox(11, this, this.mApp.mPlayerInfo.mHasUsedCheatKeys);
		...
		public override void AddedToManager(WidgetManager theWidgetManager)
		{
			...
			this.AddWidget(this.mCheatCheckBox);
		...
		public override void RemovedFromManager(WidgetManager theWidgetManager)
		{
			...
			this.RemoveWidget(this.mCheatCheckBox);
		...
		public override void Resize(int theX, int theY, int theWidth, int theHeight)
		{
			...
			this.mCheatCheckBox.Resize(theX2, (int)Constants.InvertAndScale(195f), (int)Constants.InvertAndScale(46f), (int)Constants.InvertAndScale(45f));
		...
		public override void Draw(Graphics g)
		{
			...
			int num2 = -24;
			int num3 = 14;
			TodCommon.TodDrawString(g, this.mMusicSliderOn ? "[OPTIONS_MUSIC_VOLUME]" : "[OPTIONS_MUSIC_OFF]", num2 + Constants.NewOptionsDialog_MusicLabel_X, num3 + num + newOptionsDialog_Music_Offset, Resources.FONT_DWARVENTODCRAFT18, theColor, Constants.NewOptionsDialog_VibrationLabel_MaxWidth, DrawStringJustification.DS_ALIGN_RIGHT);
			TodCommon.TodDrawString(g, "[OPTIONS_SOUND_FX]", num2 + Constants.NewOptionsDialog_FXLabel_X, num3 + Constants.NewOptionsDialog_FXLabel_Y + newOptionsDialog_FX_Offset, Resources.FONT_DWARVENTODCRAFT18, theColor, Constants.NewOptionsDialog_VibrationLabel_MaxWidth, DrawStringJustification.DS_ALIGN_RIGHT);
			TodCommon.TodDrawString(g, "[OPTIONS_VABRATION]", num2 + Constants.NewOptionsDialog_VibrationLabel_X, num3 + Constants.NewOptionsDialog_VibrationLabel_Y + newOptionsDialog_FullScreenOffset, Resources.FONT_DWARVENTODCRAFT18, theColor, Constants.NewOptionsDialog_VibrationLabel_MaxWidth, DrawStringJustification.DS_ALIGN_RIGHT);
			//TodCommon.TodDrawString(g, "[OPTIONS_RUN_LOCKED]", num2 + Constants.NewOptionsDialog_VibrationLabel_X, num3 + Constants.NewOptionsDialog_LockedLabel_Y + newOptionsDialog_FullScreenOffset, Resources.FONT_DWARVENTODCRAFT18, theColor, Constants.NewOptionsDialog_VibrationLabel_MaxWidth, DrawStringJustification.DS_ALIGN_RIGHT);
			TodCommon.TodDrawString(g, "[OPTIONS_ADDON_CHEAT]", 
				num2 + Constants.NewOptionsDialog_VibrationLabel_X, 
				num3 + Constants.NewOptionsDialog_LockedLabel_Y + newOptionsDialog_FullScreenOffset, 
				Resources.FONT_DWARVENTODCRAFT18, 
				theColor, 
				Constants.NewOptionsDialog_VibrationLabel_MaxWidth, 
				DrawStringJustification.DS_ALIGN_RIGHT);
			TodCommon.TodDrawString(g, LawnApp.AppVersionNumber, num2 + this.mWidth / 2, num3 - 3 + this.mVersionY, Resources.FONT_PICO129, theColor, DrawStringJustification.DS_ALIGN_CENTER);
		}
		...
		public override void CheckboxChecked(int theId, bool check)
		{
			...
			switch (theId) 
			{
				...
				//	LawnDialog lawnDialog = this.mApp.DoDialog(Dialogs.RESTART_WARNING, true, theDialogHeader, theDialogLines, "", 3);
				//	lawnDialog.mLawnYesButton.mLabel = TodStringFile.TodStringTranslate("[DIALOG_BUTTON_OK]");
				//	return;
				case 11:
					this.mApp.mPlayerInfo.mHasUsedCheatKeys = check;
					if (this.mApp.mGameSelector != null) 
					{
						if (check)
						{
							mApp.mGameSelector.AddWidget(mApp.mGameSelector.mChallengePageSurvivalButton);
							mApp.mGameSelector.AddWidget(mApp.mGameSelector.mChallengePageLimboButton);
						}
						else
						{
							mApp.mGameSelector.RemoveWidget(mApp.mGameSelector.mChallengePageSurvivalButton);
							mApp.mGameSelector.RemoveWidget(mApp.mGameSelector.mChallengePageLimboButton);
						}
					}
					return;
				default:
		...
		public override void ButtonDepress(int theId)
		{
			...
			case 2:
				if (this.mApp.mBoard != null)
				{
					...
					LawnDialog lawnDialog = this.mApp.DoDialog(Dialogs.DIALOG_CONFIRM_IN_GAME_RESTART, true, theDialogHeader, theDialogLines, "", 1);
					lawnDialog.mLawnYesButton.mLabel = TodStringFile.TodStringTranslate("[DIALOG_BUTTON_OK]");
			...
			case 7:
			{
				...
				LawnDialog lawnDialog2 = this.mApp.DoDialog(Dialogs.DIALOG_ABOUT, true, empty, theDialogLines2,"", 3);
				lawnDialog2.mMinWidth = (int)Constants.InvertAndScale(350f); //350f
		...
		public Checkbox mCheatCheckBox;
