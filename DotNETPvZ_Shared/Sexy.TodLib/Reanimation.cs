...
namespace Sexy.TodLib
{
    ...
    internal class Reanimation
    {
        ...
        public void Draw(Graphics g, bool isHardwareClipRequired)
        {
            this.mGetFrameTime = true;
            this.DrawRenderGroup(g, ReanimatorXnaHelpers.RENDER_GROUP_NORMAL, isHardwareClipRequired);
        }


        public void DrawRenderGroup(Graphics g, int theRenderGroup) // 2021-07-19   default($isHardwareClipRequired) = true
        {   // sometimes manually set to false for immersive effect, like in LawnApp.DrawCrazyDave()
            DrawRenderGroup(g, theRenderGroup, true);
        }

            public void DrawRenderGroup(Graphics g, int theRenderGroup, bool isHardwareClipRequired)
        {
            if (this.mDead)
            {
                return;
            ...
            for (int i = 0; i < (int)this.mDefinition.mTrackCount; i++)
            {
                ...
                {
                    bool flag = this.DrawTrack(g, i, theRenderGroup, isHardwareClipRequired);
                    if (reanimatorTrackInstance.mAttachmentID != null)
                    {
        ...
        private bool DrawTrack(Graphics g, int theTrackIndex, int theRenderGroup, bool isHardwareClipRequired)
        {
            ...
            if (reanimAtlasImage == null)
            {
                if (image != null)
                {
                    ...
                    this.ReanimBltMatrix(g, image, ref Reanimation.tempMatrix, ref trect, mTrackColor, Graphics.DrawMode.DRAWMODE_NORMAL, theSrcRect, isHardwareClipRequired);
                    if (this.mEnableExtraAdditiveDraw)
                    {
                        this.ReanimBltMatrix(g, image, ref Reanimation.tempMatrix, ref trect, theColor, Graphics.DrawMode.DRAWMODE_ADDITIVE, theSrcRect, isHardwareClipRequired);
                    }
        ...
        public void GetFrameTime(out ReanimatorFrameTime theFrameTime)
        {
            ...
            if (theFrameTime.mAnimFrameBeforeInt >= (short)(this.mFrameStart + this.mFrameCount - 1))
            {
