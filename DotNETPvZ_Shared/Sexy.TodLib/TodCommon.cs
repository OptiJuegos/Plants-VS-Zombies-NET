...
namespace Sexy.TodLib
{
    ...
    internal static class TodCommon
    {
        ...
        {
            float relativeDisplacement = 0f;
            switch (theCurve)
            {
                case TodCurves.CURVE_CONSTANT:
                    relativeDisplacement = 0f;
                    break;
                case TodCurves.CURVE_LINEAR:
                    relativeDisplacement = theTime;
                    break;
                case TodCurves.CURVE_EASE_IN:
                    relativeDisplacement = TodCommon.TodCurveQuad(theTime);
                    break;
                case TodCurves.CURVE_EASE_OUT:
                    relativeDisplacement = TodCommon.TodCurveInvQuad(theTime);
                    break;
                case TodCurves.CURVE_EASE_IN_OUT:
                    relativeDisplacement = TodCommon.TodCurveS(TodCommon.TodCurveS(theTime));
                    break;
                case TodCurves.CURVE_EASE_IN_OUT_WEAK:
                    relativeDisplacement = TodCommon.TodCurveS(theTime);
                    break;
                case TodCurves.CURVE_FAST_IN_OUT:
                    relativeDisplacement = TodCommon.TodCurveInvQuadS(TodCommon.TodCurveInvQuadS(theTime));
                    break;
                case TodCurves.CURVE_FAST_IN_OUT_WEAK:
                    relativeDisplacement = TodCommon.TodCurveInvQuadS(theTime);
                    break;
                case TodCurves.CURVE_BOUNCE:
                    relativeDisplacement = TodCommon.TodCurveBounce(theTime);
                    break;
                case TodCurves.CURVE_BOUNCE_FAST_MIDDLE:
                    relativeDisplacement = TodCommon.TodCurveQuad(TodCommon.TodCurveBounce(theTime));
                    break;
                case TodCurves.CURVE_BOUNCE_SLOW_MIDDLE:
                    relativeDisplacement = TodCommon.TodCurveInvQuad(TodCommon.TodCurveBounce(theTime));
                    break;
                case TodCurves.CURVE_SIN_WAVE:
                    relativeDisplacement = (float)Math.Sin((double)(theTime * 3.1415927f * 2f));
                    break;
                case TodCurves.CURVE_EASE_SIN_WAVE:
                    relativeDisplacement = (float)Math.Sin((double)(TodCommon.TodCurveS(theTime) * 3.1415927f * 2f));
                    break;
                default:
                    Debug.ASSERT(false);
                    break;
            ...
            return thePositionStart + (thePositionEnd - thePositionStart) * relativeDisplacement;
        }
        ...
        {
            int timeDelta = theTimeAge - theTimeStart;
            int timeInterval = theTimeEnd - theTimeStart;
            float theTime = (float)timeDelta / (float)timeInterval;
            return TodCommon.TodCurveEvaluateClamped(theTime, thePositionStart, thePositionEnd, theCurve);
        }
